	...
	br} else {
	  do {
		...
		cin >> part[i][1];
	           ...
	           (part[i][1] > dm - (part[i - 1][0] + part[i - 1][1])));
	}
	somma = part[i][0] + part[i][1];
	if ((somma == dm) && (i < np - 1)) {
	  cout << "\nNon è possibile aggiungere le partizioni rimanenti!\nIl "
	          "programma cancellera' la partizione appena aggiunta";
...
int InputProgrammi(int part[][2], int n) {
  ...
  do {
	if (p > max) {
	  cout << "Il nuovo programma non puo' richiedere piu' memoria di quella "
	          "disponibile!"
	       << endl;
	}
	cout << "==> ";
...
void InsProg(int part1[][2], int part2[][2], int i, int &npo, int &npl, int p,
  ...
  if (npo <= 50) {
	...
	cout << "\nE' stato raggiunto il limite massimo di partizioni, impossibile "
	        "crearne di nuove!";
  }
